...
def reg_user(menu_choice):
    ...
    if menu_choice == "r":  
        ...
        while new_user in usernames_list:
            ...
            print("Error! The username you entered already exists.\n")
        ...
        return("\nYour new username and password have been successfully added.")
...
def add_task(menu_choice):
    ...
    if menu_choice == "a":
        ...
        # Function to calculate the current date.
...
# Function to view all tasks assigned to logged in user.
...
_____________________________________""")  
    ...
    task_selection = int(input("""\nPlease select task number to edit or type -1 to return to the main menu. \n : """))
    ...
    for line in task_file:
            print(f'\n{task_file[task_selection]}')
            break
        
    # If they select '-1', they return to the main menu.
    if task_selection == "-1":  
    ...
    # If they enter a task number, they can choose to mark as complete or edit.        
       ...
       option = input("""\nWould you like to Mark the task as complete or Edit the task: 
                                                    M - Mark
                                                    E - Edit  
                                                    : """).upper()
       ...
       if option == "M":
           ...
           x = (task_file[task_selection])
           # Finding the position
           user_task = task_file[task_selection].strip().split(",")
           new_state = task_file[task_selection].strip().replace(user_task[5], ' Yes')
           task_file.pop(task_selection)
           ...
           with open(r"tasks.txt", 'w') as fp:
                for number, line in enumerate(l1):
                    if number not in [task_selection]:
                        fp.write(line)
                        
           # Updating tasks file and append new updated task
       ...
       # and (tasks_dict[f"Task {task_selection} details:"][5] == "No"):
       elif option == "E": 
           ...
           if edit_choice == "u":  
               ...
               # Finding the position
               user_task = task_file[task_selection].strip().split(",")
               new_state = task_file[task_selection].strip().replace(user_task[0], input("Change username \n - "))
               task_file.pop(task_selection)
               task_file.append(new_state)
               print(f'\n{new_state}\n')
               ...
               return("\nThe task username has been updated successfully.")  
           ...
           # If they choose to edit the due date, they are prompted to enter a new date. 
               ...
               # Finding the position
               user_task = task_file[task_selection].strip().split(",")
               new_state = task_file[task_selection].strip().replace(user_task[4], input("Enter a new due date (e.g. 19 May 2022) \n - "))
               task_file.pop(task_selection)
               task_file.append(new_state)
               print(f'\n{new_state}\n')
...
def over_due_check(due_date):
    ...
    # The dates in this task are in the format '10 Dec 2021' as a string.
...
# Function to Generate text files 'task_overview.txt' and 'user_overview.txt'.
...
while (username not in usernames_list) or (password not in passwords_list):
        ...
        if (username not in usernames_list) and (password in passwords_list):
            ...
            print("\nYour username is not registered - Please try again\n")
...
while 1:
    ...
    if username == "admin":  # The admin user menu.
    ...
    if menu == "r":  # 'r' = register new user function

        print(reg_user(menu))

    elif menu == "a": # 'a' = add new task function

        print(add_task(menu))

    elif menu == "va":  # va' = view all task function 

        print(view_all(menu))

    elif menu == "vm":  # 'vm' view my tasks function 

       print(view_mine(menu, username))

    elif menu == "gr":  # 'gr' = generate reports fuction
